...
브라우저용 ID 생성기와 그 바탕인 보안·재현 가능 난수 라이브러리 모노레포다. 프로젝트 기반(검증 게이트)을 갖췄고, root(재현 가능한 난수 코어·컬렉션 샘플링), `@cp949/random/state`(상태 facade), `@cp949/random/secure`(보안 난수), `@cp949/random/id`(무작위 ID·UUID·순환·카운터 ID)를 구현했다. 0.1.0은 아직 배포 전이다.

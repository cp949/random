...
| changesets                                   | 모노레포 다중 패키지 버전 동기화 도구. 단일 패키지엔 맞지 않는다. 릴리스 자동화는 `release-it`을 쓴다(9절).                                                                                                   |
...
| 7   | 에이전트 | `pnpm --filter @cp949/random release --dry-run`(`release-it`) 출력 검토(버전, 태그명, 파일 목록, `access: public`). `--` 구분자를 넣으면 안 된다 — pnpm이 하위 프로세스 argv에 `--`를 리터럴로 남겨 release-it(`node:util.parseArgs`)이 `--dry-run`을 플래그가 아닌 positional로 오인하고 `increment`에 잘못 대입한다. |
| 8   | 사용자   | `! pnpm --filter @cp949/random release`. `release-it`이 버전 bump·`git commit`·`git tag v<version>`·push·`npm publish`(OTP 프롬프트)를 한 번에 처리한다.                                                                                                                                                               |
| 9   | 에이전트 | `npm view @cp949/random@0.1.0 version dist.tarball` 확인.                                                                                                                                                                                                                                                              |
...
| publish 경로            | 0.1.0은 로컬 `npm publish`(`release-it` 경유). trusted publisher 등록은 패키지가 존재해야 가능해 첫 회는 로컬이 강제된다. CI publish는 Issue.                                                                                                                                                                                                                                                                          |
| 릴리스 도구             | `packages/random`에 `release-it` 도입(`.release-it.json`). changesets는 모노레포 다중 패키지 동기화 도구라 단일 패키지엔 안 맞아 배제하지만, 단일 저장소 버전 bump·`git tag`·`npm publish` 오케스트레이션은 release-it으로 자동화한다. changelog 자동 생성 플러그인은 켜지 않는다 — CHANGELOG `[0.1.0]` 요약 단락은 형식이 고정돼 있어 손으로 쓴다(6.4).                                                               |
| tag·release note        | `main`에 `v0.1.0`(형제 저장소 관행, `release-it`의 `git.tagName`이 생성·push). release note = CHANGELOG `[0.1.0]` 요약. GitHub Release 없음(`.release-it.json`의 `github.release: false`).                                                                                                                                                                                                                             |
...
| publish 주체·순서       | 7절. `npm login`·`main` 병합·`pnpm --filter @cp949/random release`(`npm publish`·`git tag`·push 포함)는 사용자. 에이전트는 `release --dry-run` 검토와 `npm view` 확인.                                                                                                                                                                                                                                                 |

...
브라우저에서 쓰는 ID 생성기와 그 바탕이 되는 난수 라이브러리다. UUID·nanoid·순환 ID(`./id`), 보안
난수(`./secure`), 재현 가능한 PRNG·샘플링(`.`, `./state`)을 subpath로 나눠 제공하고, 런타임 의존성은 없다.
...
"nanoid"는 UUID와 달리 표준 기구가 정한 형식이 아니다.

UUID(RFC 9562)는 자리마다 의미가 고정된 표준 형식이다. `xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx`
36자(대시 포함)에 128비트를 담지만 6비트는 버전·variant로 고정돼 있어 실제 무작위 비트는 122비트다.
이 형식을 검증·파싱하는 `isUuid`·`parseUuid`도 `./id`에 따로 있다.

nanoid는 표준이 아니라 `ai/nanoid`라는 npm 패키지가 정착시킨 방식이다. 자리마다 고정된 의미 없이
64자 알파벳(`A-Za-z0-9-_`, 문자당 6비트)에서 무작위로 뽑는다. 기본 21자면 6비트 × 21 = 126비트라
UUID v4(122비트)와 충돌 확률은 비슷하면서 대시 없이 5자 더 짧다. 이게 nanoid 형식인지 검사하는
표준 파서는 없다. 그냥 무작위 문자열이기 때문이다.

여기 있는 `nanoid()`는 `ai/nanoid` 패키지를 의존성으로 가져온 게 아니라, 그 패키지의 기본 출력
형식(길이 21, 같은 64자 알파벳)을 직접 구현으로 재현한 것이다. 난수원은 다른 `./id` 함수와 같은
`globalThis.crypto.getRandomValues`를 쓴다.
...
"보안 난수"의 근거는 자체 알고리즘이 아니라 위임이다. `./secure`와 `./id`의 무작위 함수는
`globalThis.crypto.getRandomValues`(OS·브라우저의 CSPRNG. Linux `getrandom`, Windows
...
(`createXoshiro128Source(seed)`) 같은 seed는 항상 같은 수열을 낸다. 재현이 목적이니 예측
가능한 게 정상이다. seed 없는 `rand`(`./state`)도 초기 상태 4 word만 `getRandomValues`로
뽑고 이후는 PRNG 상태 전이로 진행하므로, 연속된 출력 몇 개만 보면 내부 상태를 복원해 이후 값을
예측할 수 있다. 초기 시딩에 CSPRNG를 썼다는 사실이 출력의 예측 불가능성을 보장하지는 않는다.

`./id`는 함수마다 다르다. 여기서 "추측 가능"은 다음 값이나 다른 사용자의 값을 알아낼 수
있는가를 뜻한다. 전체 문자열의 일부(예: 무작위 부분)만 CSPRNG이고 나머지가 고정 규칙이어도,
그 무작위 부분의 엔트로피가 충분하면 전체 값은 여전히 추측 불가능하다.

| 함수                                              | 다음 값·다른 사용자의 값을 추측할 수 있는가                                                                                            |
| ------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `nanoid`, `uuidv4`, `randomId`                    | 아니오. 무작위 부분 전체가 `getRandomValues` 직결이며, `timestamp: true`를 켜도 무작위 부분의 길이·엔트로피는 그대로다                 |
...
값 추측 가능성과 별개로 생성 시각이 드러나는가의 문제도 있다. `uuidv7`과

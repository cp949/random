...
/**
 * 보안 난수를 소문자 hex 문자열로 만든다. 결과 길이는 `byteLength`의 두 배다.
 *
 ...
 * `undefined`이면 기본값 32(256비트, hex 64자)다.
 * @returns 소문자 hex 문자열. 값은 재현할 수 없다(계약이 아니다).
...
export function randomHex(byteLength = 32): string {
  ...
  return toHex(defaultRandomBytes(byteLength));
}
